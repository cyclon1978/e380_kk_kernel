...
pthread_t threadPreviewNighthawk;
sem_t semPrvThread, semPrvThreadBack, semPrvThreadEnd;
...
NighthawkPrvState NHPrvState = NH_PRV_STATE_UNINIT;

static MVOID *NighthawkPrvThread(MVOID *arg);
ImgBufQueNode NHProcessNode;
...
ASVLOFFSCREEN g_srcYv12 = {0};
ASVLOFFSCREEN g_dstYv12 = {0};

ASVLOFFSCREEN g_srcNv21 = {0};
ASVLOFFSCREEN g_dstNv21 = {0};
...
static MVOID NighthawkPreviewProcess(NighthawkPrvState state) {
	...
	if (NHProcessNode.getImgBuf() != NULL) {
		MLong ImgWidth, ImgHeight;
		ASVLOFFSCREEN srcImg, dstImg;
		
		ImgWidth = NHProcessNode.getImgBuf()->getImgWidth();
		ImgHeight = NHProcessNode.getImgBuf()->getImgHeight();
		
		if (!hEnhancer) {
        	nMemSize = 5 * 1024 * 1024 + (ImgWidth * ImgHeight * 3 / 2) * 5;
			pMem = malloc(nMemSize);
			if(MNull == pMem)
			{
				MY_LOGE("malloc error pMem == MNull");
				return;
			}
			hMemMgr = MMemMgrCreate(pMem, nMemSize);
			if(MNull == hMemMgr)
			{
				MY_LOGE("MMemMgrCreate error hMemMgr == MNull");
				SafeFree(pMem);
				return;
			}

			// call the following code only once
			if (MOK != ANH_Init(hMemMgr, &hEnhancer)) {
		...
		g_srcYv12.u32PixelArrayFormat = ASVL_PAF_YV12;
		g_srcYv12.i32Width	= ImgWidth;
		g_srcYv12.i32Height	= ImgHeight;
		g_srcYv12.pi32Pitch[0] = g_srcYv12.i32Width;
		g_srcYv12.pi32Pitch[1] = g_srcYv12.i32Width / 2;
		g_srcYv12.pi32Pitch[2] = g_srcYv12.i32Width / 2;
		g_srcYv12.ppu8Plane[0] = (MUInt8*)NHProcessNode.getImgBuf()->getVirAddr();
		g_srcYv12.ppu8Plane[1] = g_srcYv12.ppu8Plane[0] + g_srcYv12.i32Width * g_srcYv12.i32Height;
		g_srcYv12.ppu8Plane[2] = g_srcYv12.ppu8Plane[0] + g_srcYv12.i32Width * g_srcYv12.i32Height * 5 / 4;

	#ifdef NIGHT_HAWK_DEBUG
		char SrcYv12Path[256];
		sprintf(SrcYv12Path, "/mnt/sdcard/DCIM/src_%dx%d_%d.yv12",  g_srcYv12.i32Width, g_srcYv12.i32Height, g_framesIndex);
		saveRawDate(g_srcYv12, SrcYv12Path);
	#endif //NIGHT_HAWK_DEBUG

		if(MNull == g_dstYv12.ppu8Plane[0])
		{
			g_dstYv12.u32PixelArrayFormat = ASVL_PAF_YV12;
			g_dstYv12.i32Width	= ImgWidth;
			g_dstYv12.i32Height 	= ImgHeight;
			g_dstYv12.pi32Pitch[0] = g_dstYv12.i32Width;
			g_dstYv12.pi32Pitch[1] = g_dstYv12.i32Width / 2;
			g_dstYv12.pi32Pitch[2] = g_dstYv12.i32Width / 2;
			g_dstYv12.ppu8Plane[0] = (MUInt8*)malloc(g_dstYv12.i32Width * g_dstYv12.i32Height * 3 / 2);
			g_dstYv12.ppu8Plane[1] = g_dstYv12.ppu8Plane[0] + g_dstYv12.i32Width * g_dstYv12.i32Height;
			g_dstYv12.ppu8Plane[2] = g_dstYv12.ppu8Plane[0] + g_dstYv12.i32Width * g_dstYv12.i32Height * 5 / 4;
		}

	#ifdef NIGHT_HAWK_IMAGE_CONVERT
		if(MNull == g_srcNv21.ppu8Plane[0])
		{
			g_srcNv21.u32PixelArrayFormat = ASVL_PAF_NV21;
			g_srcNv21.i32Width = ImgWidth;
			g_srcNv21.i32Height = ImgHeight;
			g_srcNv21.pi32Pitch[0] = g_srcNv21.i32Width;
			g_srcNv21.pi32Pitch[1] = g_srcNv21.i32Width;
			g_srcNv21.ppu8Plane[0] = (MUInt8*)malloc(g_srcYv12.i32Width * g_srcYv12.i32Height * 3 / 2);
			g_srcNv21.ppu8Plane[1] = g_srcNv21.ppu8Plane[0] + g_srcNv21.i32Width * g_srcNv21.i32Height;
		}

		
		Yv12ToNv21(g_srcNv21, g_srcYv12);

		#ifdef NIGHT_HAWK_DEBUG
			char SrcNv21Path[256];
			sprintf(SrcNv21Path, "/mnt/sdcard/DCIM/src_%dx%d_%d.nv21",  g_srcNv21.i32Width, g_srcNv21.i32Height, g_framesIndex);
			saveRawDate(g_srcNv21, SrcNv21Path);
		#endif //NIGHT_HAWK_DEBUG
	
		if(MNull == g_dstNv21.ppu8Plane[0])
		{
			g_dstNv21.u32PixelArrayFormat = g_srcNv21.u32PixelArrayFormat;
			g_dstNv21.i32Width = g_srcNv21.i32Width;
			g_dstNv21.i32Height = g_srcNv21.i32Height;
			g_dstNv21.pi32Pitch[0] = g_srcNv21.pi32Pitch[0];
			g_dstNv21.pi32Pitch[1] = g_srcNv21.pi32Pitch[1];
			g_dstNv21.ppu8Plane[0] = (MUInt8*)malloc(g_dstNv21.i32Width * g_dstNv21.i32Height * 3 / 2);
			g_dstNv21.ppu8Plane[1] = g_dstNv21.ppu8Plane[0] + g_dstNv21.i32Width * g_dstNv21.i32Height;
		}
		
		MY_LOGD("ANH_Enhance--->In hEnhancer=%p, g_param.lIntensity=%d, g_param.lDenoise=%d", 
			hEnhancer, g_param.lIntensity, g_param.lDenoise);
		ret = ANH_Enhance(hEnhancer, &g_srcNv21, &g_dstNv21, &g_param);		
		if(MOK != ret)
		{
			MY_LOGE("ANH_Enhance process error ret = %d", ret);
			SafeFree(g_srcNv21.ppu8Plane[0]);
			SafeFree(g_dstNv21.ppu8Plane[0]);
			return;
		}

		Nv21ToYv12(g_dstYv12, g_dstNv21);
	
		#ifdef NIGHT_HAWK_DEBUG
			char DstNv21Path[256];
			sprintf(DstNv21Path, "/mnt/sdcard/DCIM/dst_%dx%d_%d.nv21",  g_dstNv21.i32Width, g_dstNv21.i32Height, g_framesIndex);
			saveRawDate(g_dstNv21, DstNv21Path);
		#endif //NIGHT_HAWK_DEBUG
		...
		MY_LOGD("ANH_Enhance--->In hEnhancer=%p, g_param.lIntensity=%d, g_param.lDenoise=%d", 
			hEnhancer, g_param.lIntensity, g_param.lDenoise);
		ret = ANH_Enhance(hEnhancer, &g_srcYv12, &g_dstYv12, &g_param);
		if(MOK != ret)
		{
			MY_LOGE("ANH_Enhance process error ret = %d", ret);
			return;
		...
		sprintf(DstYv12Path, "/mnt/sdcard/DCIM/dst_%dx%d_%d.yv12",  g_dstYv12.i32Width, g_dstYv12.i32Height, g_framesIndex);
		saveRawDate(g_dstYv12, DstYv12Path);
	#endif //NIGHT_HAWK_DEBUG

		memcpy(NHProcessNode.getImgBuf()->getVirAddr(), g_dstYv12.ppu8Plane[0], g_dstYv12.i32Width*g_dstYv12.i32Height*3/2);
	}
	globalcacheFlushAll();
	return;
...
static MVOID *NighthawkPrvThread(MVOID *arg)
{
	...
	while (NHPrvState != NH_PRV_STATE_UNINIT) {
		::sem_wait(&semPrvThread);  
		MY_LOGD("Got semPrvThread. NHPrvState: %d.", NHPrvState);
		if(NHProcessNode.getImgBuf() != NULL){
			NighthawkPreviewProcess(NHPrvState);
		}
...
start()
{
	...
	pthread_create(&threadPreviewNighthawk, NULL, NighthawkPrvThread, NULL);
	NHPrvState = NH_PRV_STATE_INIT;
	MY_LOGD("[init] NighthawkPrvThread: %d.", threadPreviewNighthawk);
#endif
...
stop()
{
...
#if NIGHT_HAWK_PREVIEW_THREAD       		
	MY_LOGD("[Stop start] NighthawkPrvThread: %d.", threadPreviewNighthawk);
	NHPrvState = NH_PRV_STATE_UNINIT;
	MY_LOGD("[Stop] NighthawkPrvThread: %d.", threadPreviewNighthawk);

	if(MNull != hEnhancer)
	{
		ANH_Uninit(&hEnhancer);
		hEnhancer = MNull;
	}
	...
	SafeFree(g_dstYv12.ppu8Plane[0]);
...
updateOne()
{
    ...
    if (flag & eID_Pass2DISPO)
    {
	    ...
	    if (eShotMode_BrightMagicShot == mspParamsMgr->getShotMode()) { 
			...
			NHPrvState = NH_PRV_STATE_PROCESS;
			NHProcessNode = dispNode;
