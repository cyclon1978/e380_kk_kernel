...
#define NIGHT_HAWK_PREVIEW_THREAD   (1)
#endif

#if NIGHT_HAWK_PREVIEW_THREAD
...
pthread_t threadZsdPreviewNighthawk;
sem_t semZsdPrvThread, semZsdPrvThreadBack, semZsdPrvThreadEnd;

typedef enum {
	NH_PRV_STATE_INIT,			
	NH_PRV_STATE_PROCESS, 
	NH_PRV_STATE_UNINIT,			
} NighthawkPrvState;
NighthawkPrvState NHZsdPrvState = NH_PRV_STATE_UNINIT;

static MVOID *NighthawkPrvThread(MVOID *arg);
ImgBufQueNode NHZsdProcessNode;
ImgBufQueNode ZsdNULLNode;

//#define NIGHT_HAWK_DEBUG
//#define NIGHT_HAWK_IMAGE_CONVERT
#define SafeFree(p) {if ((p)) free(p); (p) = MNull; }

MLong nZsdMemSize = 0;
MVoid *pZsdMem = MNull;
MHandle hZsdMemMgr = MNull;
MHandle hZsdEnhancer = MNull;
ANH_PARAM g_Zsdparam = {0};
int	g_ZsdframesIndex = 0;

ASVLOFFSCREEN g_ZsdsrcYv12 = {0};
ASVLOFFSCREEN g_ZsddstYv12 = {0};

ASVLOFFSCREEN g_ZsdsrcNv21 = {0};
ASVLOFFSCREEN g_ZsddstNv21 = {0};


/******************************************************************************
*
*******************************************************************************/
static long getNHtime()
{
	struct timeval time;
...
*
*******************************************************************************/
static void Nv21ToYv12(ASVLOFFSCREEN& dstImg, ASVLOFFSCREEN& srcImg)
{
	dstImg.u32PixelArrayFormat = ASVL_PAF_YV12;
	dstImg.i32Width = srcImg.i32Width;
	dstImg.i32Height = srcImg.i32Height;
	dstImg.pi32Pitch[0] = dstImg.i32Width;
	dstImg.pi32Pitch[1] = dstImg.i32Width / 2;
	dstImg.pi32Pitch[2] = dstImg.i32Width / 2;
	memcpy(dstImg.ppu8Plane[0], srcImg.ppu8Plane[0], dstImg.i32Width*dstImg.i32Height);
	MUInt8* srcV = srcImg.ppu8Plane[0] + srcImg.i32Width*srcImg.i32Height;
	MUInt8* srcU = srcV + 1;
	MUInt8* dstV = dstImg.ppu8Plane[0] + dstImg.i32Width*dstImg.i32Height;
	MUInt8* dstU = dstV + dstImg.i32Width*dstImg.i32Height / 4;

	long length = dstImg.i32Width*dstImg.i32Height / 4;
...
*
*******************************************************************************/
static void Yv12ToNv21(ASVLOFFSCREEN& dstImg, ASVLOFFSCREEN& srcImg)
{
	...
	memcpy(dstImg.ppu8Plane[0], srcImg.ppu8Plane[0], dstImg.i32Width*dstImg.i32Height);
	MUInt8* srcV = srcImg.ppu8Plane[0] + srcImg.i32Width*srcImg.i32Height;
	MUInt8* srcU = srcV + srcImg.i32Width*srcImg.i32Height / 4;
	MUInt8* dstV = dstImg.ppu8Plane[0] + dstImg.i32Width*dstImg.i32Height;
	MUInt8* dstU = dstV + 1;
...
*
*******************************************************************************/
static void saveRawDate(ASVLOFFSCREEN& frame, char path[])
{
	...
	switch(frame.u32PixelArrayFormat)
	{
		case ASVL_PAF_NV21:
			fwrite(frame.ppu8Plane[0], frame.pi32Pitch[0], frame.i32Height, file);
			fwrite(frame.ppu8Plane[1], frame.pi32Pitch[1], frame.i32Height / 2, file);
		break;
		
		case ASVL_PAF_YV12:
			fwrite(frame.ppu8Plane[0], frame.pi32Pitch[0], frame.i32Height, file);
	   		fwrite(frame.ppu8Plane[1], frame.pi32Pitch[1], frame.i32Height / 2, file);
	    	fwrite(frame.ppu8Plane[2], frame.pi32Pitch[2], frame.i32Height / 2, file);
		break;	
	}
...
*
*******************************************************************************/
static MVOID NighthawkPreviewProcess(NighthawkPrvState state) {
	MY_LOGD("NighthawkPreviewProcess");
	if (NHZsdProcessNode.getImgBuf() != NULL) {
		MLong ImgWidth, ImgHeight;
		ASVLOFFSCREEN srcImg, dstImg;
		
		ImgWidth = NHZsdProcessNode.getImgBuf()->getImgWidth();
		ImgHeight = NHZsdProcessNode.getImgBuf()->getImgHeight();
		
		if (!hZsdEnhancer) {
        	nZsdMemSize = 5 * 1024 * 1024 + (ImgWidth * ImgHeight * 3 / 2) * 5;
			pZsdMem = malloc(nZsdMemSize);
			if(MNull == pZsdMem)
			{
				MY_LOGE("malloc error pZsdMem == MNull");
				return;
			}
			hZsdMemMgr = MMemMgrCreate(pZsdMem, nZsdMemSize);
			if(MNull == hZsdMemMgr)
			{
				MY_LOGE("MMemMgrCreate error hZsdMemMgr == MNull");
				SafeFree(pZsdMem);
				return;
			}

			// call the following code only once
			if (MOK != ANH_Init(hZsdMemMgr, &hZsdEnhancer)) {
				MY_LOGE("ANH_Init error");
				if(hZsdMemMgr != MNull)
				{
					MMemMgrDestroy(hZsdMemMgr);
					hZsdMemMgr = MNull;
				}
			...
			ANH_GetDefaultParam(&g_Zsdparam);
			g_ZsdframesIndex = 0;
		}

	#ifdef NIGHT_HAWK_DEBUG
		g_ZsdframesIndex ++;
	#endif //NIGHT_HAWK_DEBUG

		int ret = MOK;

		g_ZsdsrcYv12.u32PixelArrayFormat = ASVL_PAF_YV12;
		g_ZsdsrcYv12.i32Width	= ImgWidth;
		g_ZsdsrcYv12.i32Height	= ImgHeight;
		g_ZsdsrcYv12.pi32Pitch[0] = g_ZsdsrcYv12.i32Width;
		g_ZsdsrcYv12.pi32Pitch[1] = g_ZsdsrcYv12.i32Width / 2;
		g_ZsdsrcYv12.pi32Pitch[2] = g_ZsdsrcYv12.i32Width / 2;
		g_ZsdsrcYv12.ppu8Plane[0] = (MUInt8*)NHZsdProcessNode.getImgBuf()->getVirAddr();
		g_ZsdsrcYv12.ppu8Plane[1] = g_ZsdsrcYv12.ppu8Plane[0] + g_ZsdsrcYv12.i32Width * g_ZsdsrcYv12.i32Height;
		g_ZsdsrcYv12.ppu8Plane[2] = g_ZsdsrcYv12.ppu8Plane[0] + g_ZsdsrcYv12.i32Width * g_ZsdsrcYv12.i32Height * 5 / 4;

	#ifdef NIGHT_HAWK_DEBUG
		char SrcYv12Path[256];
		sprintf(SrcYv12Path, "/mnt/sdcard/DCIM/src_%dx%d_%d.yv12",  g_ZsdsrcYv12.i32Width, g_ZsdsrcYv12.i32Height, g_ZsdframesIndex);
		saveRawDate(g_ZsdsrcYv12, SrcYv12Path);
	#endif //NIGHT_HAWK_DEBUG

		if(MNull == g_ZsddstYv12.ppu8Plane[0])
		{
			g_ZsddstYv12.u32PixelArrayFormat = ASVL_PAF_YV12;
			g_ZsddstYv12.i32Width	= ImgWidth;
			g_ZsddstYv12.i32Height 	= ImgHeight;
			g_ZsddstYv12.pi32Pitch[0] = g_ZsddstYv12.i32Width;
			g_ZsddstYv12.pi32Pitch[1] = g_ZsddstYv12.i32Width / 2;
			g_ZsddstYv12.pi32Pitch[2] = g_ZsddstYv12.i32Width / 2;
			g_ZsddstYv12.ppu8Plane[0] = (MUInt8*)malloc(g_ZsddstYv12.i32Width * g_ZsddstYv12.i32Height * 3 / 2);
			g_ZsddstYv12.ppu8Plane[1] = g_ZsddstYv12.ppu8Plane[0] + g_ZsddstYv12.i32Width * g_ZsddstYv12.i32Height;
			g_ZsddstYv12.ppu8Plane[2] = g_ZsddstYv12.ppu8Plane[0] + g_ZsddstYv12.i32Width * g_ZsddstYv12.i32Height * 5 / 4;
		}

	#ifdef NIGHT_HAWK_IMAGE_CONVERT
		if(MNull == g_ZsdsrcNv21.ppu8Plane[0])
		{
			g_ZsdsrcNv21.u32PixelArrayFormat = ASVL_PAF_NV21;
			g_ZsdsrcNv21.i32Width = ImgWidth;
			g_ZsdsrcNv21.i32Height = ImgHeight;
			g_ZsdsrcNv21.pi32Pitch[0] = g_ZsdsrcNv21.i32Width;
			g_ZsdsrcNv21.pi32Pitch[1] = g_ZsdsrcNv21.i32Width;
			g_ZsdsrcNv21.ppu8Plane[0] = (MUInt8*)malloc(g_ZsdsrcYv12.i32Width * g_ZsdsrcYv12.i32Height * 3 / 2);
			g_ZsdsrcNv21.ppu8Plane[1] = g_ZsdsrcNv21.ppu8Plane[0] + g_ZsdsrcNv21.i32Width * g_ZsdsrcNv21.i32Height;
		}

		
		Yv12ToNv21(g_ZsdsrcNv21, g_ZsdsrcYv12);

		#ifdef NIGHT_HAWK_DEBUG
			char SrcNv21Path[256];
			sprintf(SrcNv21Path, "/mnt/sdcard/DCIM/src_%dx%d_%d.nv21",  g_ZsdsrcNv21.i32Width, g_ZsdsrcNv21.i32Height, g_ZsdframesIndex);
			saveRawDate(g_ZsdsrcNv21, SrcNv21Path);
		#endif //NIGHT_HAWK_DEBUG
	
		if(MNull == g_ZsddstNv21.ppu8Plane[0])
		{
			g_ZsddstNv21.u32PixelArrayFormat = g_ZsdsrcNv21.u32PixelArrayFormat;
			g_ZsddstNv21.i32Width = g_ZsdsrcNv21.i32Width;
			g_ZsddstNv21.i32Height = g_ZsdsrcNv21.i32Height;
			g_ZsddstNv21.pi32Pitch[0] = g_ZsdsrcNv21.pi32Pitch[0];
			g_ZsddstNv21.pi32Pitch[1] = g_ZsdsrcNv21.pi32Pitch[1];
			g_ZsddstNv21.ppu8Plane[0] = (MUInt8*)malloc(g_ZsddstNv21.i32Width * g_ZsddstNv21.i32Height * 3 / 2);
			g_ZsddstNv21.ppu8Plane[1] = g_ZsddstNv21.ppu8Plane[0] + g_ZsddstNv21.i32Width * g_ZsddstNv21.i32Height;
		}
		
		MY_LOGD("ANH_Enhance--->In hZsdEnhancer=%p, g_Zsdparam.lIntensity=%d, g_Zsdparam.lDenoise=%d", 
			hZsdEnhancer, g_Zsdparam.lIntensity, g_Zsdparam.lDenoise);
		ret = ANH_Enhance(hZsdEnhancer, &g_ZsdsrcNv21, &g_ZsddstNv21, &g_Zsdparam);		
		if(MOK != ret)
		{
			MY_LOGE("ANH_Enhance process error ret = %d", ret);
			SafeFree(g_ZsdsrcNv21.ppu8Plane[0]);
			SafeFree(g_ZsddstNv21.ppu8Plane[0]);
			return;
		}

		Nv21ToYv12(g_ZsddstYv12, g_ZsddstNv21);
	
		#ifdef NIGHT_HAWK_DEBUG
			char DstNv21Path[256];
			sprintf(DstNv21Path, "/mnt/sdcard/DCIM/dst_%dx%d_%d.nv21",  g_ZsddstNv21.i32Width, g_ZsddstNv21.i32Height, g_ZsdframesIndex);
			saveRawDate(g_ZsddstNv21, DstNv21Path);
		#endif //NIGHT_HAWK_DEBUG
		...
		MY_LOGD("ANH_Enhance--->In hZsdEnhancer=%p, g_Zsdparam.lIntensity=%d, g_Zsdparam.lDenoise=%d", 
			hZsdEnhancer, g_Zsdparam.lIntensity, g_Zsdparam.lDenoise);
		ret = ANH_Enhance(hZsdEnhancer, &g_ZsdsrcYv12, &g_ZsddstYv12, &g_Zsdparam);
		if(MOK != ret)
		{
			MY_LOGE("ANH_Enhance process error ret = %d", ret);
			return;
		}	
	#endif //NIGHT_HAWK_IMAGE_CONVERT

	#ifdef NIGHT_HAWK_DEBUG
		char DstYv12Path[256];
		sprintf(DstYv12Path, "/mnt/sdcard/DCIM/dst_%dx%d_%d.yv12",  g_ZsddstYv12.i32Width, g_ZsddstYv12.i32Height, g_ZsdframesIndex);
		saveRawDate(g_ZsddstYv12, DstYv12Path);
	#endif //NIGHT_HAWK_DEBUG

		memcpy(NHZsdProcessNode.getImgBuf()->getVirAddr(), g_ZsddstYv12.ppu8Plane[0], g_ZsddstYv12.i32Width*g_ZsddstYv12.i32Height*3/2);
	}
	globalcacheFlushAll();
	return;
...
static MVOID *NighthawkPrvThread(MVOID *arg)
{
	::prctl(PR_SET_NAME,"NighthawkPreviewThread", 0, 0, 0);   // Give this thread a name.
	...
	while (NHZsdPrvState != NH_PRV_STATE_UNINIT) {
		::sem_wait(&semZsdPrvThread);  
		MY_LOGD("Got semZsdPrvThread. NHZsdPrvState: %d.", NHZsdPrvState);
		if(NHZsdProcessNode.getImgBuf() != NULL){
			NighthawkPreviewProcess(NHZsdPrvState);
		}
		::sem_post(&semZsdPrvThreadBack);
	}

	::sem_post(&semZsdPrvThreadEnd);
   
	NHZsdProcessNode = ZsdNULLNode;
...
#if NIGHT_HAWK_PREVIEW_THREAD
		::sem_init(&semZsdPrvThread, 0, 0);
		::sem_init(&semZsdPrvThreadBack, 0, 0);
		::sem_init(&semZsdPrvThreadEnd, 0, 0);
		pthread_create(&threadZsdPreviewNighthawk, NULL, NighthawkPrvThread, NULL);
		NHZsdPrvState = NH_PRV_STATE_INIT;
		MY_LOGD("[init] NighthawkPrvThread: %d.", threadZsdPreviewNighthawk);
#endif
...
stop()
{
    FUNCTION_IN;
...
#if NIGHT_HAWK_PREVIEW_THREAD       		
		MY_LOGD("[Stop start] NighthawkPrvThread: %d.", threadZsdPreviewNighthawk);
		NHZsdPrvState = NH_PRV_STATE_UNINIT;
		MY_LOGD("[Stop] NighthawkPrvThread: %d.", threadZsdPreviewNighthawk);
	
		if(MNull != hZsdEnhancer)
		{
			ANH_Uninit(&hZsdEnhancer);
			hZsdEnhancer = MNull;
		}
		...
		SafeFree(g_ZsddstYv12.ppu8Plane[0]);
...
updateOne()
{
    ...
    { 
	#if NIGHT_HAWK_PREVIEW_THREAD
	    MY_LOGD("mspParamsMgr->getShotMode()=%d",mspParamsMgr->getShotMode());
	    if (eShotMode_BrightMagicShot == mspParamsMgr->getShotMode()) { 
			MY_LOGD("Nighthawk start");
			NHZsdPrvState = NH_PRV_STATE_PROCESS;
			NHZsdProcessNode = dispNode;
			::sem_post(&semZsdPrvThread);
			::sem_wait(&semZsdPrvThreadBack);			
			MY_LOGD("Nighthawk end");
	    }
	#endif
