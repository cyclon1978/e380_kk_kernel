...
U32 bq24158_i2c_write (U8 chip, U8 *cmdBuffer, int cmdBufferLen, U8 *dataBuffer, int dataBufferLen)
{
    ...
    {
        print("[bq24158_i2c_write] exceed I2C FIFO length!! \n");
        return 0;
    }
...
kal_uint32 bq24158_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
  ...
  //  print("[bq24158_read_interface] Reg[%x]=0x%x\n", RegNum, data);
    
    data &= (MASK << SHIFT);
    *val = (data >> SHIFT);
    
    //print("[bq24158_read_interface] val=0x%x\n", *val);
...
kal_uint32 bq24158_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    cmd = RegNum;
    result_tmp = bq24158_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);
   // print("[bq24158_config_interface] Reg[%x]=0x%x\n", RegNum, data);

    data &= ~(MASK << SHIFT);
    data |= (val << SHIFT);

    result_tmp = bq24158_i2c_write(chip_slave_address, &cmd, cmd_len, &data, data_len);
    //print("[bq24158_config_interface] write Reg[%x]=0x%x\n", RegNum, data);

    // Check
    result_tmp = bq24158_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);
    //print("[bq24158_config_interface] Check Reg[%x]=0x%x\n", RegNum, data);
...
void bq24158_pre_init(void)
{
unsigned char i;
 kal_uint8 ret_val=0;
 bq24158_read_interface(0x06,&ret_val,0xFF,0x0);
 if(ret_val != 0x77)
    bq24158_config_interface_reg(0x06,0x77);
	//bq24158_config_interface_reg(0x00,0x80);
	//bq24158_config_interface_reg(0x04,0x81);
	else
		{
...
//power on vibrate teng
    pmic_config_interface(DIGLDO_CON40,0x06,PMIC_RG_VIBR_VOSEL_MASK,PMIC_RG_VIBR_VOSEL_SHIFT);
    pmic_config_interface(DIGLDO_CON39,0x01, PMIC_RG_VIBR_EN_MASK,PMIC_RG_VIBR_EN_SHIFT);
    mdelay(100);
    pmic_config_interface(DIGLDO_CON39,0x00, PMIC_RG_VIBR_EN_MASK,PMIC_RG_VIBR_EN_SHIFT);
