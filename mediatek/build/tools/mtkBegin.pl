...
#if (!-e $flag_subrel)
{
    ...
    if(-e "$binaryAppPath/.filter")
    {
        system ("mv $binaryAppPath/.filter mediatek/build/tools/.filter");
    }

    my @lines;

    if(-e "mediatek/build/tools/.filter"){
        open(my $in, "<", "mediatek/build/tools/.filter") or die "Can't open mediatek/build/tools/.filter: $!";
        @lines = <$in>;
        close $in or die "$in: $!";
    }
	...
	foreach my $app (@applist)
	{
      ...
      foreach my $linesapp (@lines)         
      {
        my $totalstring = $binaryAppPath . "/" . $linesapp ;
        my $appstring = $app . "\n" ;
   
        if($appstring eq $totalstring)
        {
            $same = 1;
            last;
        }        
      ...
      if($same == 1)
      {
         #print "Signing  signature for $app \n";
         &signApk($app);
      }
	}
	if (-d $srcDir)
