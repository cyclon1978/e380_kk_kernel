...
static volatile int slid_enable_status = 0;
...
#if KPD_HAS_SLIDE_QWERTY
static int slid_eint_reg=0;
static ssize_t kpd_store_slid_state(struct device_driver *ddri, const char *buf, size_t count)
{
	if (sscanf(buf, "%u", &slid_enable_status) != 1) {
			kpd_print("kpd slid state: Invalid values\n");
			return -EINVAL;
		}

	switch(slid_enable_status)
    	{
        	case 0 :
			mt65xx_eint_mask(KPD_SLIDE_EINT);
			kpd_print("kpd slid state: KPD_SLIDE_EINT Mask!\n");
     		break;
		case 1 :
			if(slid_eint_reg==0)
			{
			...
			kpd_print("kpd slid state:  KPD_SLIDE_EINT UNMask!\n");
		break;
            
		default:
   			kpd_print("kpd slid state: erro!\n");
        	break;
  	}
	return count;
}

static ssize_t kpd_show_slid_state(struct device_driver *ddri, char *buf)
{
	ssize_t res;
	res = snprintf(buf, PAGE_SIZE, "%d\n", slid_enable_status);     
	return res;   
...
static DRIVER_ATTR(kpd_slid_state,	S_IWUSR | S_IRUGO,	kpd_show_slid_state,	kpd_store_slid_state);
#endif

static struct driver_attribute *kpd_attr_list[] = {
	&driver_attr_kpd_call_state,
#if KPD_HAS_SLIDE_QWERTY
	&driver_attr_kpd_slid_state,	
#endif
};
