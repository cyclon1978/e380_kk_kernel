...
static void hwmsen_work_func(struct work_struct *work)
{
	...
	{
	    if(1 == atomic_read(&hwm_obj->power_suspend))
	    {
	       // slow down polling rate at power suspend  let system have chance to sleep
	       mod_timer(&obj->timer, jiffies + (HZ/2));
		   HWM_LOG("hwm_dev power suspend work polling\n");
	    }
		else
		{
