...
static void mag_early_suspend(struct power_suspend *h);
static void mag_late_resume(struct power_suspend *h);
...
static int mag_probe(struct platform_device *pdev) 
{
	...
	register_power_suspend(&mag_context_obj->early_drv);
