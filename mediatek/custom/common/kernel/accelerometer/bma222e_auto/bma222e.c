...
static struct i2c_board_info __initdata i2c_BMA222E={ I2C_BOARD_INFO("BMA222E", (0x30>>1))};
...
static int BMA222E_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	printk("BMA222E_SetPowerMode ok!,enable=%d\r\n",enable);
	mdelay(20);
...
/*----------------------------------------------------------------------------*/
static int BMA222E_SW_RESET(struct i2c_client *client)
{
	u8 databuf[10];   
	int err;	
	printk("BMA222E:BMA222E_SW_RESET\r\n");
struct bma222e_i2c_data *obj = i2c_get_clientdata(client);
	databuf[0] = 0x14;  //BGW_SOFTRESET
	databuf[1] = 0xB6;
	mutex_lock(&obj->i2c_mutex);
	...
	{
		printk("BMA222E:initialize client fail!!\n");
		return -1;        
	}
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = BMA222E_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
...
//printk("BMA222E_late_resume \r\n");
	...
	BMA222E_power(obj->hw, 1);
	if(err = BMA222E_SW_RESET(obj->client))
	{
		printk("BMA222E:initialize client fail!!\n");	
		return; 	       
	}
...
static int bma222e_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma222e_gsensor_operate;  // auto detect
...
static int  bma222e_local_init(void)
{
	...
	BMA222E_power(hw, 1);
	//bma222e_force[0] = hw->i2c_num;  //yanfang 
	if(i2c_add_driver(&bma222e_i2c_driver))
	{
		GSE_ERR("add driver error\n");
		return -1;
